...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t meloflavio/portal-uft-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t meloflavio/portal-uft-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name portal-uft-backend-acceptance -d meloflavio/portal-uft-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name portal-uft-frontend-acceptance --link portal-uft-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d meloflavio/portal-uft-frontend:acceptance
